...
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
        ...
        var url = "${ctxStatic}/ckfinder/ckfinder.html?type=${ctype}&startupPath=${ctype}:/"+year+"/"+month+
                "${uploadPath}/&startupFolderExpanded=true&action=js&func=${input}SelectAction&thumbFunc=${input}ThumbSelectAction&cb=${input}Callback&dts=${type eq 'thumb'?'1':'0'}&sm=${selectMultiple?1:0}";
