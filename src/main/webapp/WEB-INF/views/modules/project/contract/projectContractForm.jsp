		...
		<table class="table-form">
			...
			<tr>
				...
				<td colspan="1" >
						...
						<sys:treeselect id="apply" name="apply.id"  value="${projectContract.apply.id}" labelName="apply.projectName" labelValue="${projectContract.apply.projectName}"
							title="立项名称" url="/apply/external/projectApplyExternal/treeData?proMainStage=20" cssClass="required"  allowClear="true" notAllowSelectParent="true"/>
					...
					$("#applyButton, #applyName").click(function(){
						...
						top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("/apply/external/projectApplyExternal/treeData?proMainStage=11,21,30,31")+"&proMainStage=03&module=&checked=&extId=&isAll=", "选择项目名称", 300, 420, {
							ajaxData:{selectIds: $("#applyId").val()},buttons:{"确定":"ok", "清除":"clear", "关闭":true}, submit:function(v, h, f){
								if (v=="ok"){
									...
									var url ="${ctx }/apply/external/projectApplyExternal/proApply4contract?id="+ids.join(",").replace(/u_/ig,"");
								    $.ajax( {  
								        type : "get",  
								        url : url,  
								        dataType:"json",
								        success : function(apply) {
								            ...
								            $("#customer_contact_name").text(apply.customerContactName);
								            $("#customer_phone").text(apply.customerPhone);
