...
<form:form id="inputForm" modelAttribute="projectInvoice" htmlEscape="false"
    ...
    <table class="table-form">
        ...
        <tr>
            ...
            <c:choose>
            <c:when test="${projectInvoice.act.taskDefKey eq 'usertask_invoice'}">
        <tr>
        ...
        <tr>
            ...
            <td>
                <fmt:formatDate value="${projectInvoice.returnDate}" pattern="yyyy-MM-dd"/>
            </td>
        </tr>
    ...
    <div class="">
        ...
        <tr id="invoiceList{{idx}}">
            ...
            <td class="input-medium">{{row.invoiceDate}}</td>
        </tr>//-->
        </script>
        ...
        <c:choose>
            <c:when test="${projectInvoice.act.taskDefKey eq 'usertask_return'}">
                <tr>
                    <td colspan="2">
                        <a href="javascript:" onclick="addRow('#returnList', rowReturnIdx, returnFormTpl); rowReturnIdx = rowReturnIdx + 1;"
                           class="btn">新增</a>
                    </td>
                </tr>
            </c:when>
        ...
        <%--回款表单--%>
        <script type="text/template" id="returnFormTpl">//<!--
        <tr id="returnList{{idx}}">
            <input id="returnList{{idx}}_id" name="returnList[{{idx}}].id" type="hidden" value="{{row.id}}"/>
            <input id="returnList{{idx}}_delFlag" name="returnList[{{idx}}].delFlag" type="hidden" value="0"/>
            ...
            <td>
                ...
                <input id="returnList{{idx}}_amount" name="returnList[{{idx}}].amount" style="width:85%" type="text"
            ...
            <td>
                <input id="returnList{{idx}}_returnDate" style="width:120px" name="returnList[{{idx}}].returnDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="{{row.returnDate}}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
			</td>

            <td class="text-center" width="10">
                {{#delBtn}}<span class="close" onclick="delRow(this, '#returnList{{idx}}')" title="删除">&times;</span>{{/delBtn}}
            </td>
        </tr>//-->
        </script>
        ...
        <table id="contentTable" class="table table-striped table-bordered table-condensed">
            ...
            <tr>
                ...
                <th>单价(含税)</th>
                <th>金额(含税)</th>
                <%--<th>本次结转成本金额</th><td> {{row.costAmount}} </td>--%>
                <shiro:hasPermission name="project:invoice:edit">
                    <th width="10">&nbsp;</th>
                </shiro:hasPermission>
            </tr>
        ...
        <script type="text/javascript">
            ...
            var returnFormTpl = $("#returnFormTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
            ...
            $(document).ready(function() {
                ...
                // 默认新增一行，让财务专员填写回款信息
//                addRow('#returnList', rowReturnIdx, returnFormTpl, dataReturn[i]);
//                rowReturnIdx++;
    ...
    <div class="form-actions">
        ...
        <c:choose>
            <c:when test="${projectInvoice.act.taskDefKey eq 'usertask_return'}">
                <input id="btnSubmit" class="btn btn-primary" type="submit" value="保存" onclick="$('#flag').val('save')"/>
            </c:when>
            <c:otherwise>
            </c:otherwise>
        </c:choose>
