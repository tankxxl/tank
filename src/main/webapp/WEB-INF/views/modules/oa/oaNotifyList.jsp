...
<body>
	...
	<form:form id="searchForm" modelAttribute="oaNotify" htmlEscape="false"
			   action="${ctx}/oa/oaNotify/${oaNotify.self?'self':''}" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>标题：</label>
				<form:input path="title" maxlength="200" class="input-medium"/>
			</li>
			<li><label>类型：</label>
			...
			<c:if test="${!requestScope.oaNotify.self}">
				<li><label>状态：</label>
				<form:radiobuttons path="status" items="${fns:getDictList('oa_notify_status')}" itemLabel="label" itemValue="value"/>
				</li>
			</c:if>
			...
			<tr>
				...
				<c:if test="${!requestScope.oaNotify.self}">
					...
					<c:if test="${oaNotify.status ne '1'}">
						...
						<a href="${ctx}/oa/oaNotify/send?id=${oaNotify.id}">发送</a>
					</c:if>
