...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		...
		// 查询列表，所有的跟流程相关的变量都是在此传递到业务对象中的，再传递到jsp中。再传递到后台。
...
//		return ProcessDiagramGenerator.generateDiagram(bpmnModel, "png", activeActivityIds);
	 ...
	 * @return	封装了各种节点信息
	 */
	public List<Map<String, Object>> traceProcess(String procInsId) throws Exception {
		Execution execution = runtimeService.createExecutionQuery().executionId(procInsId).singleResult();//执行实例
		...
		ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity) ((RepositoryServiceImpl) repositoryService)
				.getDeployedProcessDefinition(procIns.getProcessDefinitionId());
		...
		for (ActivityImpl activity : activitiList) {
			...
			Map<String, Object> activityImageInfo = packageSingleActivitiInfo(activity, procIns, currentActiviti);
     ...
     */
    @Transactional(readOnly = false)
    public void endProcess(Act act) {
        try {
            processCustomService.endProcess(act.getTaskId());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     */
    @Transactional(readOnly = false)
    public String startProcessEatFirstTask(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
    ...
    /**
     * rgz
     * 获取流转历史列表，用于打印表单，只显示通过的审批节点。
     * @param procInsId 流程实例
     * @param startAct 开始活动节点名称
     * @param endAct 结束活动节点名称
     */
    public List<Act> histoicFlowListPass(String procInsId, String startAct, String endAct){
        List<Act> actList = Lists.newArrayList();
        if (StringUtils.isBlank(procInsId)) {
            return actList;
        }
