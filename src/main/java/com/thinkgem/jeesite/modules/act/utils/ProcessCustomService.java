...
public class ProcessCustomService extends BaseService {
     ...
     * 
     */
    public void jumpTo(Act act, String activityId, Map<String, Object> vars) throws Exception {
    	if (StringUtils.isEmpty(activityId)) {
    		throw new Exception("驳回目标节点ID为空！");
    	}
    	...
    	// 添加意见
 		...
 		backProcess(taskId, activityId, vars);
    }
    
    public void jumpToOwner(Act act, Map<String, Object> vars) {
    	try {
			jumpTo(act, UserTaskType.UT_OWNER, vars);
		} catch (Exception e) {
			e.printStackTrace();
		}
    }
